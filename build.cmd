...
:: Compile the sketch
set build_command=arduino-cli compile --verbose --fqbn esp8266:esp8266:generic:%board_params% --export-binaries --build-property compiler.cache_core=false --build-property mkbuildoptglobals.extra_flags="--no_cache_core" --build-property "--build.opt.flags=" --build-property build.extra_flags="-Wall %build_flags%" --build-path "%VSCA_BUILD_DIR%" --build-cache-path "%VSCA_BUILD_DIR%" --jobs 0 --log-level trace --log-file "%log_file%" %arduino_cli_flags%
