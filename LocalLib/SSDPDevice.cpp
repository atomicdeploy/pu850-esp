	...
	_respondToAddr(0,0,0,0)
{
	...
	sprintf(m_schemaURL, "description.xml"); // ssdp/schema.xml
...
void SSDPDeviceClass::send(ssdp_send_parameters_t *parameters) {
	...
	int len = snprintf_P(buffer, sizeof(buffer),
		...
		m_interval, m_modelName, m_modelNumber, usn1, usn2, usn3, parameters->type == RESPONSE ? "ST" : "NT", uri,
	...
	parameters->time = parameters->type == NOTIFY_ALIVE ? parameters->time + m_interval * 900L : 0; // 1000 ms - 100 ms
}

void SSDPDeviceClass::schema(Print &client) const {
	IPAddress ip = WiFi.localIP();
	char buffer[strlen_P(SSDP_SCHEMA_TEMPLATE) + 1];
	strcpy_P(buffer, SSDP_SCHEMA_TEMPLATE);
	client.printf(buffer,
		ip.toString().c_str(), m_port,
		m_deviceType,
		m_friendlyName,
		m_presentationURL,
		m_serialNumber,
...
void SSDPDeviceClass::_onTimerStatic(SSDPDeviceClass* self) {
	// self->update();
}

void SSDPDeviceClass::_startTimer() {
	_stopTimer();
	_timer = new SSDPTimer();
	ETSTimer* tm = &(_timer->timer);
	const int interval = 1000;
	os_timer_disarm(tm);
	os_timer_setfn(tm, reinterpret_cast<ETSTimerFunc*>(&SSDPDeviceClass::_onTimerStatic), reinterpret_cast<void*>(this));
	os_timer_arm(tm, interval, 1 /* repeat */);
}

void SSDPDeviceClass::_stopTimer() {
	if(!_timer)
		return;
